...
if (fs.existsSync(bpManifestPath)) {
  try {
    ...
    console.log(`🔼 [MiningUtility] バージョンを更新します: [${newVersion.join(', ')}]`);
    ...
    console.log('✅ [MiningUtility] manifest.json を更新しました。');
  } catch (error) {
    console.error('[MiningUtility] manifest.json の更新中にエラーが発生しました:', error);
    process.exit(1);
  }
} else {
  console.warn(`⚠️ [MiningUtility] manifest.json が見つかりませんでした: ${bpManifestPath}`);
}
...
async function buildPackages() {
  console.log('📦 [MiningUtility] パッケージング処理を開始します...');
  try {
    const mcaddonPath = path.join(distDir, 'MiningUtility.mcaddon');
    const bpPackPath = path.join(distDir, 'MiningUtility_BP.mcpack');
    const rpPackPath = path.join(distDir, 'MiningUtility_RP.mcpack');
    ...
    // localDistDir にもコピー
    [
      [mcaddonPath, path.join(localDistDir, 'MiningUtility.mcaddon')],
      [bpPackPath, path.join(localDistDir, 'MiningUtility_BP.mcpack')],
      [rpPackPath, path.join(localDistDir, 'MiningUtility_RP.mcpack')],
    ].forEach(([src, dest]) => {
    ...
    console.log('\n🎉 [MiningUtility] パッケージの生成が完了しました！');
    if (fs.existsSync(mcaddonPath)) {
      console.log(`👉 一括インポート用: dist/MiningUtility.mcaddon`);
    }
    console.log(`👉 個別インポート用: dist/MiningUtility_BP.mcpack`);
    if (fs.existsSync(rpPackPath)) {
      console.log(`👉 個別インポート用: dist/MiningUtility_RP.mcpack`);
    }
    console.log('');
  } catch (error) {
    console.error('[MiningUtility] 圧縮中にエラーが発生しました:', error);
    process.exit(1);
  }
}
